...
segmentation, a dedicated management network for device access, a dedicated HQ server
subnet, and inter-VLAN routing so departments can communicate through controlled paths.
...
**Action:** Built a multi-VLAN HQ campus (VLANs 100/200/300/400/999) with proper trunk
pruning, router-on-a-stick inter-VLAN routing, deliberate STP root election with backup
roots, and SSH-only management access restricted to a dedicated management VLAN.
...
- VLANs (100, 200, 300, 400, 999, 1000)
...
|-------|---------|-----------------|
| VLANs exist | `show vlan brief` | VLANs 100,200,300,400,999,1000 active |
| Trunks up | `show interfaces trunk` | All uplinks trunking, native 1000 |
| Inter-VLAN routing | `ping 10.1.200.10` from PC-ENG1 | Success |
| STP root correct | `show spanning-tree root` | DSW1=root VLAN100/400/999, DSW2=root VLAN200/300 |
...
This Project 01 README has been aligned to the validated current HQ VLAN design used by
later projects in the repo. The HQ server subnet is VLAN 400 (`10.1.40.0/24`).
