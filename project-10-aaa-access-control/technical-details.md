...
|------|----------|-------------|-----------------|
| admin | <TACACS-USER-PASSWORD> | netadmin | 15 |
...
- `test aaa group tacacs+ admin <TACACS-USER-PASSWORD> new-code` → `User successfully authenticated`
...
Local fallback to `admin / <TACACS-USER-PASSWORD>` succeeded. TACACS was unreachable → IOS returned an error → fell through to `local`. ✅
